...
class StripeModule(PaymentModuleBase):
    ...
    def get_payment_template(self):
        ...
        return "stripe_payment/payment_form.html"
    ...
    def get_admin_config_data(self):
        ...
        config = self._get_config()
        return {
            'stripe_public_key': config.get('stripe_public_key', '') if config else '',
            'stripe_secret_key': config.get('stripe_secret_key', '') if config else '',
            'webhook_secret': config.get('webhook_secret', '') if config else '',
            'is_test_mode': config.get('is_test_mode', True) if config else True,
        }
